...
pub async fn main() -> Result<(), Error> {
    ...
    println!(
        "Here is first value that we are writing to the first test key: {:?}",
        &hello_value.clone().into_string().unwrap()
    );

    println!(
        "Here is the second value that we are writing to the second test key: {:?}",
        &world_value.clone().into_string().unwrap()
    );
    ...
    println!(
        "Here is key/value pair retrieved from the Key Value Store: {:?}:{:?}",
        &first_test_key.clone(),
        &response.clone().unwrap().as_str().unwrap()
    );
    println!(
        "Here is key/value pair retrieved from the Key Value Store: {:?}:{:?}",
        &second_test_key.clone(),
        &_response2.clone().unwrap().as_str().unwrap()
    );
